...
// 高雅的文件占用提示文言文 ("此物正萦绕于尘世之务，暂不可去。请解其羁绊，再行超度。")
const wchar_t* TEXT_IN_USE = L"\u6B64\u7269\u6B63\u8426\u7ED5\u4E8E\u5C18\u4E16\u4E4B\u52A1\uFF0C\u6682\u4E0D\u53EF\u53BB\u3002\u8BF7\u89E3\u5176\u7F81\u7ECA\uFF0C\u518D\u884C\u8D85\u5EA6\u3002";
...
// 占用状态控制：当文件被占用时切换为专属显示模式，总时长延长至 10 秒 (约 330 帧)
...
void RenderFrame(HWND hwnd, HDC hdcBase) {
    ...
    if (isFileInUse) {
        ...
        RECT titleRect = { 0, cy - 100, width, height };
        DrawTextW(hdcMem, TEXT_LOCKING, -1, &titleRect, DT_CENTER | DT_TOP | DT_SINGLELINE);
        DeleteObject(hTitleFont);

        // 绘制正中央高雅的文言文提示（金色文字）
        SetTextColor(hdcMem, RGB(255, 215, 0));
        HFONT hBodyFont = CreateFontW(42, 0, 0, 0, FW_SEMIBOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET, 
                                    OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, 
                                    DEFAULT_PITCH, L"Microsoft YaHei");
        SelectObject(hdcMem, hBodyFont);
        RECT bodyRect = { 100, cy - 20, width - 100, height };
    ...
    // ------------------------------------------------------------------
        ...
        if (result != 0) {
            // 如果文件被占用，不弹窗，直接开启 10 秒长效动画提示模式！
            isFileInUse = true;
            tick = 0; // 重置计时器，开始计算 10 秒
            BitBlt(hdcBase, 0, 0, width, height, hdcMem, 0, 0, SRCCOPY);
            SelectObject(hdcMem, hOld);
            DeleteObject(hbmMem);
            DeleteDC(hdcMem);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: 
            ...
            SetTimer(hwnd, 1, 30, NULL); // 每 30 毫秒刷新一帧
            break;
        ...
        case WM_TIMER:
            tick++;
            // 如果处于文件占用模式，则计时 330 帧 (约 10 秒) 后退出；否则按常规帧数退出
